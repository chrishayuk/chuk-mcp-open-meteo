...
# Run the MCP server
run:
	@echo "Running Open-Meteo MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_open_meteo.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_open_meteo.server; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-open-meteo:latest .
	@echo "Docker image built: chuk-mcp-open-meteo:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8000:8000 chuk-mcp-open-meteo:latest
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-open-meteo==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-open-meteo==$$version"
